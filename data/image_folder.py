...
def make_dataset(dir, max_dataset_size=float("inf"), style_dataset=False):
    images = []
    assert os.path.isdir(dir) or os.path.islink(dir), (
        "%s is not a valid directory" % dir
    )

    if style_dataset == False:
        ...
        for style_dir in sorted(os.listdir(dir)):
            if style_dir in [
                "Minimalism",
            ...
            for root, _, fnames in sorted(
                os.walk(os.path.join(dir, style_dir), followlinks=True)
            ):
                for fname in fnames:
                    if is_image_file(fname):
                        path = os.path.join(root, fname)
