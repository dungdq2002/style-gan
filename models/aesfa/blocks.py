    ...
    def forward(self, x):
        if self.type == "first":
            ...
            if self.is_dw:
                ...
                hf, lf = self.H2H(hf) + self.L2H(self.upsample(lf)), self.L2L(
                    lf
...
class AdaOctConv(nn.Module):
    def __init__(
        ...
        self.AdaConv_h = AdaConv2d(
            in_channels=h_in, out_channels=h_in, n_groups=n_groups_h
        )
        self.AdaConv_l = AdaConv2d(
            in_channels=l_in, out_channels=l_in, n_groups=n_groups_l
        )
