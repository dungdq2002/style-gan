...
from . import MSP, net, networks
from .aesfa.aesfa import AesFA
from .aesfa.networks import define_network
from .base_model import BaseModel


class AesfaCastModel(BaseModel):
    @staticmethod
    def modify_commandline_options(parser, is_train=True):
        """Configures options specific for CAST"""
        parser.add_argument("--CAST_mode", type=str, default="AesFa", choices="CAST")
        parser.add_argument(
            "--lambda_GAN_G_A",
            type=float,
            default=0.1,
        ...
        parser.add_argument(
            ...
            type=float,
            default=0.1,
            help="weight for GAN loss：GAN(G(Is, Ic))",
        )
        ...
        parser.add_argument(
            ...
            type=float,
            default=0.05,
            help="weight for NCE loss: NCE(G(Ic, Is), Is)",
        )
        parser.add_argument(
            "--lambda_NCE_D",
            type=float,
            default=1.0,
            help="weight for NCE loss: NCE(I, I+, I-)",
        )
        ...
        parser.add_argument(
            ...
            type=int,
            default=64,
            help="number of filters in the first layer of the generator",
        )
        ...
        parser.add_argument(
            ...
            type=float,
            default=0.5,
            help="input ratio of low-frequency channel",
        )
        ...
        parser.add_argument(
            ...
            type=float,
            default=0.5,
            help="output ratio of low-frequency channel",
        )
        ...
        parser.add_argument(
            ...
            type=int,
            default=[1, 1],
            help="frequency ratio of low-frequency channel",
        )
        ...
        parser.add_argument(
            ...
            type=int,
            default=3,
            help="kernel size of style encoder",
        )
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            self.model_names = ["Aesfa", "Dec_A", "Dec_B", "D", "P_style", "D_A", "D_B"]
        else:  # during test time, only load G
            self.model_names = ["Aesfa", "Dec_A", "Dec_B"]
        ...
        self.netAesfa = AesFA(opt)

        self.netDec_A = define_network("Generator", config=opt)
        self.netDec_B = define_network("Generator", config=opt)
        init_net(self.netAesfa, "normal", 0.02, self.gpu_ids)
        init_net(self.netDec_A, "normal", 0.02, self.gpu_ids)
        init_net(self.netDec_B, "normal", 0.02, self.gpu_ids)

        if self.isTrain:
            style_vgg = MSP.vgg
            style_vgg.load_state_dict(torch.load("pretrained/style_vgg.pth"))
            ...
            self.nce_loss = MSP.InfoNCELoss(
                opt.temperature, opt.hypersphere_dim, opt.queue_size
            ).to(self.device)
            self.mse_loss = nn.MSELoss()

            self.patch_sampler = K.RandomResizedCrop(
                (256, 256), scale=(0.8, 1.0), ratio=(0.75, 1.33)
            ).to(self.device)

            self.criterionCyc = torch.nn.L1Loss().to(self.device)
            self.optimizer_G = torch.optim.Adam(
                itertools.chain(
                    self.netAesfa.parameters(),
                    self.netDec_A.parameters(),
                    self.netDec_B.parameters(),
                ),
                lr=opt.lr_G,
            ...
            self.optimizer_D_NCE = torch.optim.Adam(
                itertools.chain(self.netD.parameters(), self.netP_style.parameters()),
                lr=opt.lr_D_NCE,
                betas=(opt.beta1, opt.beta2),
            )
    ...
    def optimize_parameters(self):
        ...
        if self.opt.lambda_NCE_D > 0.0:
            ...
            self.set_requires_grad(
                [self.netAesfa, self.netDec_A, self.netDec_B, self.netD_A, self.netD_B],
                False,
            )
        ...
        # update G
        self.set_requires_grad(
            [self.netD, self.netP_style, self.netD_A, self.netD_B], False
        )
        self.set_requires_grad([self.netAesfa, self.netDec_A, self.netDec_B], True)
    ...
    def forward(self):
        ...
        self.fake_B, _, _ = self.netDec_B(self.real_A_feat, self.real_B_feat)
        # print("fake_B:", len(self.fake_B), type(self.fake_B))
        if self.isTrain:
            self.real_B_feat, self.real_A_feat = self.netAesfa(
                self.real_B, self.real_A
            )  # G_A(A)
            self.fake_A, _, _ = self.netDec_A(self.real_B_feat, self.real_A_feat)
            if self.opt.lambda_CYC > 0.0:
                self.rec_A_feat, self.rec_B_feat = self.netAesfa(
                    self.fake_B, self.real_A
                )
                # print("test")
                self.rec_A, _, _ = self.netDec_A(self.rec_A_feat, self.rec_B_feat)

                self.rec_B_feat, self.rec_A_feat = self.netAesfa(
                    self.fake_A, self.real_B
                )
                self.rec_B, _, _ = self.netDec_B(self.rec_B_feat, self.rec_A_feat)
    ...
    def backward_D_NCEloss(self):
        ...
        for x in self.nce_layers:
            ...
            loss_D_cont_B += self.nce_loss(
                query_B[num], query_Bx[num], "real_B{:d}".format(x)
            )
            num += 1
    ...
    def backward_D(self):
        ...
        if self.opt.lambda_GAN_D_B > 0.0:
            ...
            self.loss_D_B = (
                self.backward_D_basic(self.netD_B, self.real_A, self.real_B, fake_B)
                * self.opt.lambda_GAN_D_B
            )
        else:
            self.loss_D_B = 0
        ...
        if self.opt.lambda_GAN_D_A > 0.0:
            ...
            self.loss_D_A = (
                self.backward_D_basic(self.netD_A, self.real_B, self.real_A, fake_A)
                * self.opt.lambda_GAN_D_A
            )
    ...
    def compute_G_loss(self):
        ...
        if self.opt.lambda_GAN_G_A > 0.0:
            ...
            self.loss_G_A = (
                self.criterionGAN(pred_fakeB, True).mean() * self.opt.lambda_GAN_G_A
            )
        else:
            self.loss_G_A = 0.0
        ...
        if self.opt.lambda_GAN_G_B > 0.0:
            ...
            self.loss_G_B = (
                self.criterionGAN(pred_fakeA, True).mean() * self.opt.lambda_GAN_G_B
            )
        else:
            self.loss_G_B = 0.0
        ...
        if self.opt.lambda_NCE_G > 0.0:
            ...
            query_A = self.netP_style(
                self.netD(fake_A, self.nce_layers), self.nce_layers
            )
            query_B = self.netP_style(
                self.netD(fake_B, self.nce_layers), self.nce_layers
            )
            ...
            for x in self.nce_layers:
                ...
                self.loss_G_NCE_style_B += self.nce_loss(
                    query_B[num], key_B[num], "real_B{:d}".format(x)
                )
                num += 1
        else:
            self.loss_G_NCE_style_A = 0
            self.loss_G_NCE_style_B = 0
        self.loss_G_NCE_style = (
            (self.loss_G_NCE_style_A + self.loss_G_NCE_style_B)
            * 0.5
            * self.opt.lambda_NCE_G
        )
        ...
        if self.opt.lambda_CYC > 0.0:
            ...
            self.loss_cyc_B = (
                self.criterionCyc(self.rec_B, self.real_B) * self.opt.lambda_CYC
            )
        else:
            self.loss_cyc_A = 0
            self.loss_cyc_B = 0
        ...
        self.loss_G = (
            self.loss_cyc
            + self.loss_G_NCE_style
            + (self.loss_G_A + self.loss_G_B) * 0.5
        )
